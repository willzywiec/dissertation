...
Estimate <- function(bn, data.set, ensemble.model, ensemble.size, sample.size) {
  ...
  bn.data <- cpdist(
    ...
    evidence = (as.integer(mass) > 275) & (as.integer(rad) > 7.62),
    # evidence = TRUE,
  ...
  vol[vol <= bn.data$mass / pu.density] <- bn.data$mass / pu.density
  ...
  # scale data
  bn.df$mass <- scale(bn.df$mass, center = data.set$training.mean[1], scale = data.set$training.sd[1])
  bn.df$rad <- scale(bn.df$rad, center = data.set$training.mean[2], scale = data.set$training.sd[2])
  bn.df$thk <- scale(bn.df$thk, center = data.set$training.mean[3], scale = data.set$training.sd[3])
  bn.df$vol <- scale(bn.df$vol, center = data.set$training.mean[4], scale = data.set$training.sd[4])
  bn.df$conc <- scale(bn.df$conc, center = data.set$training.mean[5], scale = data.set$training.sd[5])
