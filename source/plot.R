...
Plot <- function(file.name, history) {
  ...
  ggplot(history.df, aes(x = epoch)) +
    ...
    geom_point(aes(x = which.min(history.df$mae), y = min(history.df$mae), color = 'training minimum')) +
  ...
  ggsave(paste0(file.name, '.png'), dpi = 2000, height = 4, width = 6.5) %>% suppressMessages()
