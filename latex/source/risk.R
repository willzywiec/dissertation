...
Risk <- function(bn, dataset, ensemble.model, ensemble.size, sample.size, source.dir, test.dir) {

  source(paste0(source.dir, '/estimate.R'))

  risk.dir <- paste0(test.dir, '/risk/', dist, '-', risk.pool, sub('.', '', sample.size))
  ...
  if (file.exists('risk.csv')) {
    ...
    for (i in 1:risk.pool) {
      bn.data[[i]] <- Estimate(bn, dataset, ensemble.model, ensemble.size, sample.size)
