...
#
# form:  plutonium
#          alpha-phase plutonium (95% Pu-239, 5% Pu-240)
#          delta-phase plutonium (95% Pu-239, 5% Pu-240)
#          plutonium oxide (95% Pu-239, 5% Pu-240)
#        uranium
#          highly enriched uranium (93.15% U-235, 6.85% U-238)
#          uranium oxide (93.15% U-235, 6.85% U-238)
#
# mod:   aluminum oxide
#        beryllium
#        beryllium oxide
...
Build <- function(mass, form, mod, rad, ref, thk, shape, ht) {
  ...
  if (vol <= fiss.mass / fiss.density) {
    mod <- 'none'
    vol <- fiss.mass / fiss.density
    if (shape == 'sph') {
      rad <- (3/4 * vol / pi)^(1/3)
    } else if (shape == 'rcc') {
      rad <- (vol / ht / pi)^(1/2)
  ...
  mod.mass <- mod.density * (vol - fiss.mass / fiss.density)

  # calculate average density (g/cc)
  avg.density <- (fiss.mass + mod.mass) / vol

  # calculate weighted mass fractions
  fiss.wt <- fiss.mass / (fiss.mass + mod.mass)
  mod.wt <- mod.mass / (fiss.mass + mod.mass)
  ...
  if (form == 'puo2') {
    o2 = -1.18003643801257e-01
  } else if (form == 'uo2') {
    o2 = -1.19705702866899e-01
  } else {
    o2 = 0
  }
  ...
  if (form == 'alpha' || form == 'delta') {
    matl.cards <- paste0(
      '\n\t  94239.80c ', (-0.95 * fiss.wt) %>% Format(), ' $ Pu-239',
    ...
    matl.cards <- paste0(
      '\n\t  92235.80c ', (-0.9315 * fiss.wt) %>% Format(), ' $ U-235',
      '\n\t  92238.80c ', (-0.0685 * fiss.wt) %>% Format(), ' $ U-238')
  } else if (form == 'uo2') {
    matl.cards <- paste0(
      '\n\t  92235.80c ', -8.19994137779484e-01 %>% Format(), ' $ U-235', 
      '\n\t  92238.80c ', -6.03001593536175e-02 %>% Format(), ' $ U-238')
  }

  if (mod == 'al2o3') {
    matl.cards <- paste0(
      '\nm1   8016.80c ', (-4.70679444984350e-01 * mod.wt - o2 * fiss.wt) %>% Format(), ' $ O-16',
    ...
    matl.cards <- paste0(
      ...
      '\n\t   8016.80c ', (-1.18003643801257e-01 * fiss.wt) %>% Format(), ' $ O-16',
      matl.cards,
      '\nmt1    be.20t')
  } else if (mod == 'beo') {
    matl.cards <- paste0(
      '\nm1   4009.80c ', (-3.60384984537289e-01 * mod.wt) %>% Format(), ' $ Be-9',
      '\n\t   8016.80c ', (-6.39615015462711e-01 * mod.wt - o2 * fiss.wt) %>% Format(), ' $ O-16',
      matl.cards,
      '\nmt1  be-o.20t',
      '\n\t   o-be.20t')
    ...
    if (form == 'puo2' || form == 'uo2') {
      matl.cards <- paste0(
        ...
        '\n\t   8016.80c ', (o2 * fiss.wt) %>% Format(), ' $ O-16',
        matl.cards,
        '\nmt1  grph.20t')
    } else {
      matl.cards <- paste0(
    ...
    matl.cards <- paste0(
      '\nm1   8016.80c ', (-3.96896476983704e-01 * mod.wt - o2 * fiss.wt) %>% Format(), ' $ O-16',
    ...
    matl.cards <- paste0(
      ...
      '\n\t   8016.80c ', (-5.67953140203876e-01 * mod.wt - o2 * fiss.wt) %>% Format(), ' $ O-16',
    ...
    matl.cards <- paste0(
      '\nm1   8016.80c ', (-5.32481915390405e-01 * mod.wt - o2 * fiss.wt) %>% Format(), ' $ O-16',
    ...
    matl.cards <- paste0(
      ...
      '\n\t   8016.80c ', (-8.88085126727636e-01 * mod.wt - o2 * fiss.wt) %>% Format(), ' $ O-16',
      matl.cards,
      '\nmt1  lwtr.20t')
  } else if (mod == 'none') {
    if (form == 'puo2' || form == 'uo2') {
