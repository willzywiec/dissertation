...
NN <- function(ensemble.size) {
  ...
  for (i in 1:ensemble.size) {
    ...
    test.results[[i]] <- ensemble.model[[i]] %>% predict(test.df)
  }

  cat('Training MAE = ', mean(mae) %>% sprintf('%.5f', .), '\n', sep = '')
  cat('Validation MAE = ', mean(val.mae) %>% sprintf('%.5f', .), '\n', sep = '')
  cat('Test MAE = ', mean(abs(rowMeans(as.data.frame(test.results, col.names = 1:length(ensemble.model))) - test.data$keff)) %>% sprintf('%.5f', .), '\n', sep = '')
