...
path <- 'C:/Users/Will/Dropbox/Dissertation/Journal Paper/'
...
ggsave('accident-1.png', dpi = 320, height = 3, width = 6.5)
...
ggsave('accident-2.png', dpi = 320, height = 3, width = 6.5)
...
ggsave('accident-3.png', dpi = 320, height = 3, width = 6.5)
...
df <- read.csv(paste0(path, 'uncertainty.csv'), fileEncoding = 'UTF-8-BOM', header = TRUE)
...
df <- read.csv(paste0(path, 'fixed.csv'), fileEncoding = 'UTF-8-BOM', header = TRUE)
...
df1 <- read.csv(paste0(path, 'nn-1.csv'), fileEncoding = 'UTF-8-BOM', header = TRUE)
df2 <- read.csv(paste0(path, 'nn-2.csv'), fileEncoding = 'UTF-8-BOM', header = TRUE)

new.theme <- theme_gray() + theme(axis.text = element_text(color = 'black', size = 10), text = element_text(color = 'black', family = 'serif', size = 10))

theme_set(new.theme)

ggplot(df1, aes(x = layers, y = val.mae)) +
  geom_point(size = 0.9) +
  geom_line(data = df2, aes(x = layers, y = val.mae), color = '#FC4E07', size = 0.5) +
  geom_point(data = df2, aes(x = layers, y = val.mae), color = '#FC4E07', size = 0.9) +
  xlab('hidden layers') +
  ylab(bquote(mean ~ absolute ~ error ~ (k[eff]))) +
  scale_x_continuous(breaks = 1:6, minor_breaks = 1:6) +
  scale_y_log10() +

ggsave('nn.png', dpi = 320, height = 3.6, width = 6.5)
