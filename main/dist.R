...
for (i in 1:6) {
  ...
  # dim.a[[i]] <- dim[[i]]
  # dim.b[[i]] <- dim[[i]][which(sapply(dim[[i]], function(x) x <= (2 * 2.54)))] # 2 in
  # dim.c[[i]] <- dim[[i]][which(sapply(dim[[i]], function(x) x <= 0))] # 0 in
  # dim.d[[i]] <- dim[[i]][which(sapply(dim[[i]], function(x) x <= (0.25 * 2.54)))] # 0.25 in
  # dim.e[[i]] <- dim[[i]][which(sapply(dim[[i]], function(x) x <= (0.25 * 2.54)))] # 0.25 in
  # dim.m[[i]] <- dim[[i]][which(sapply(dim[[i]], function(x) x <= (0.3 * 2.54)))] # 0.3 in
  # dim.p[[i]] <- dim[[i]]
...
for (i in 1:6) {
  ...
  # exp.p[[i]] <- dnorm(seq(0, 1000, 1), mean = mean.p, sd = 25)
