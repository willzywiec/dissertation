...
thk.x <- list(thk.a, thk.b, thk.c, thk.d, thk.e, thk.m, thk.p)
...
for (i in 1:6) {
  ...
  for (j in 1:7) {
    ...
    if (dist.type == "gamma") {
      ...
      thk.fit[[j]] <- fitdist(thk.x[[j]][[i]], distr = "gamma", method = "mle")
      ...
      thk.fit[[j]] <- fgev(thk.x[[j]][[i]], method = "BFGS", std.err = FALSE)
      ...
      thk.fit[[j]] <- fitdist(thk.x[[j]][[i]], distr = "lnorm", method = "mle")
      ...
      thk.fit[[j]] <- fitdist(thk.x[[j]][[i]], distr = "norm", method = "mle")
      ...
      thk.fit[[j]] <- pareto.fit(thk.x[[j]][[i]])
      ...
      thk.fit[[j]] <- fitdist(thk.x[[j]][[i]], distr = "weibull", method = "mle")
  ...
  if (dist.type == "gev") {
    ...
    for (j in 1:7) {
      ...
      # thk.ks <- thk.ks + gofTest(thk.dist[[j]], distribution = "gev", test = "ks")$statistic[[1]]
      # thk.cvm <- thk.cvm + gofTest(thk.dist[[j]], distribution = "gev", test = "cvm")$statistic[[1]]
      # thk.ad <- thk.ad + gofTest(thk.dist[[j]], distribution = "gev", test = "ad")$statistic[[1]]
    ...
    for (j in 1:7) {
      ...
      thk.ks <- thk.ks + gofstat(thk.fit[[j]])$ks
      thk.cvm <- thk.cvm + gofstat(thk.fit[[j]])$cvm
      thk.ad <- thk.ad + gofstat(thk.fit[[j]])$ad
  ...
  ks <- (mass.ks + rad.ks + thk.ks + ht.ks) / 28
  cvm <- (mass.cvm + rad.cvm + thk.cvm + ht.cvm) / 28
  ad <- (mass.ad + rad.ad + thk.ad + ht.ad) / 28
