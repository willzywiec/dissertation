...
These scripts build a deep neural network metamodel coupled with a Bayesian network, which can be used to estimate the risk of accidents and safety incidents happening in a nuclear facility (e.g., P(keff) >= 1 🤯). The default configuration of these scripts generate approximately 85 GB of data, which are saved to the **test** folder. The hardware and software configuration I used is provided below.  
