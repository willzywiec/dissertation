...
These scripts build a Bayesian network coupled with a deep neural network metamodel of Monte Carlo radiation transport simulations, which can be used to estimate nuclear accident risk (i.e., P(keff) >= 1 🤯). The default configuration of these scripts generate approximately 85 GB of data, which are saved to the **test** folder. The hardware and software I used is provided below.  
